...
export function activate(context: vscode.ExtensionContext) {
	  ...
	  const disposable = vscode.commands.registerCommand('extension.tagTechDebt', async () => {
		...
		const selectedProject = await vscode.window.showQuickPick(jiraProjectKeys, {
            placeHolder: 'Select a Jira project',
            canPickMany: false
        });
        if (!selectedProject) {
            vscode.window.showErrorMessage('No project selected. You must pick a project to use CherryBomb');
            return;
        }
